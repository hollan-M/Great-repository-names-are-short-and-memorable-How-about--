...
window.setTimeout(()=>{
    ...
    // element creation
    let randomFont = [""]
    create_text(`      •  "Son im crine 😭" - WAS written on the board`, quotes_div as HTMLDivElement, (parent: HTMLDivElement, text: HTMLSpanElement[])=>{
        let inc_wait = 0;
        for (const element of text) {
            element.style.userSelect = "none";
            let addition = random_int(25, 50);
    ...
    create_text(`      •  "Catholic school should see me now." - Astrid, considerably more "woke" now.`, quotes_div as HTMLDivElement, (parent: HTMLDivElement, text: HTMLSpanElement[])=>{
        let inc_wait = 0;
        for (const element of text) {
            element.style.userSelect = "none";
            let addition = random_int(25, 50);
    ...
    create_text(`      •  "Despite everything, it's still you." - Toby T. Fox`, quotes_div as HTMLDivElement, (parent: HTMLDivElement, text: HTMLSpanElement[])=>{
        let inc_wait = 0;
        for (const element of text) {
